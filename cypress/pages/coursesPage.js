...
export default class profilePage extends BasePage {
    ...
    assertCoursesDetails() {
        cy.get(this.courseCards).each((card) => {
            ...
            cy.wrap(card).find(this.courseCardData.textForDurationModules).eq(0).should('be.visible')
            cy.wrap(card).find(this.courseCardData.textForDurationModules).eq(1).should('be.visible')
        })
    }
}
